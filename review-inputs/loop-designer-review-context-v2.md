...
User request: second cross-review after revisions addressing runner capability gate, evaluator contract, source attribution, cost boundary split, and write safeguards.
...
This skill is based on the loop model from "Getting started with loops": an AI agent repeatedly runs a work cycle until a predefined stop condition is met. Source read for this skill: WeChat translation "从零开始玩转循环 (Getting started with loops)【译】", 宝玉AI, 2026-07-07, https://mp.weixin.qq.com/s/lG4WU5sSykOVu4XMHtWq1g. Treat platform-specific command names from that article as concepts unless the current environment exposes those commands.
...
- Stop: the goal is met, the turn/attempt limit is reached, or the platform-specific blocked policy says to stop.
...
- Runner: the concrete mechanism that will wake the agent.
- Persistence: where loop state, last run, and dedupe keys live.
- Logs: where each run records input, action, verifier result, and errors.
- Cancel path: how the user stops or pauses the loop.
- Credentials: what account or token is used, and what it can access.
- Write permissions: what the loop may modify.
- Recovery: what happens after crash, reboot, network failure, or repeated verifier failure.
